...
#if defined(NDEBUG)
# error "Bitcoincap cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa3f945c4173b10903070286caa3161d2eb75b8dcec0865f85b4ece35873ea69c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoincap: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoincap Signed Message:\n";
...
{
    // Bitcoincap: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50000); // Bitcoincap: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bitcoincap: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Bitcoincap: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoincap: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoincap: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa67b86f62332d0fc227a3fc5b7b746110f1997f1b8e622eed68107ae64117f4d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 21/04/20 Created Bitcoin Cap ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04407327135a9d90cc14719d481c4fef08b806d898ba31695b43188c193b9622753ba24dbc7e367cca61899cf0256340e45070f53188ed775f9152612d8816a596") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1587490055;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089489795;

        if (fTestNet)
        {
            block.nTime    = 1587490034;
            block.nNonce   = 386919375;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcd8bd9f90e17734f5676cfcf04d80129b827092faf28fc0ba3ba53327d1436f4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Bitcoincap: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoincapMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoincapMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoincapMiner(CWallet *pwallet)
{
    printf("BitcoincapMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoincap-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoincapMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoincapMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoincapMiner, pwallet));
}
